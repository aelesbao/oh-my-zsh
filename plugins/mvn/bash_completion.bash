_mvn()
{
    ...
    java_options="-Dmaven.test.skip=true|-DskipTests|-DskipITs|-Dmaven.surefire.debug|-DenableCiProfile|-Dpmd.skip=true|-Dcheckstyle.skip=true|-Dtycho.mode=maven|-Dmaven.javadoc.skip=true"
    ...
    if [[ ${cur} == -D* ]] ; then
      COMPREPLY=( $(compgen -S ' ' -W "${java_options}" -- ${cur}) )
      ...
      COMPREPLY=( $(compgen -W "${long_opts}" -S ' ' -- ${cur}) )
        ...
        for plugin in $common_plugins; do
          if [[ ${cur} == ${plugin}:* ]]; then
            ...
            COMPREPLY=( $(compgen -W "${(P)var_name}" -S ' ' -- ${cur}) )
          fi
        done
